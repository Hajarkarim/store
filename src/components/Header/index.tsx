...
const Header = () => {
  ...
  return (
    <header className={`fixed left-0 top-0 w-full z-50 bg-white transition-all ${stickyMenu ? "shadow" : ""}`}>
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0 flex justify-between items-center py-4">
        {/* Logo */}
        <Link href="/">
          <Image src="/images/logo/jubasef-logo.png" alt="Logo" width={149} height={16} />
        </Link>
          ...
          {userName ? (
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-dark font-medium">Bienvenue, {userName}!</p>
              </div>
              <button
                onClick={handleLogout}
                className="px-5 py-2.5 bg-red text-white rounded-md border border-brown-dark 
                         hover:bg-red-dark hover:shadow-2 transition-all duration-200 
                         ease-out font-medium text-custom-sm flex items-center gap-2"
              >
            ...
            <Link 
              href="/signin" 
              className="flex items-center gap-2 text-dark font-medium hover:text-brown transition-colors duration-200"
            >
              <svg
                width="24"
                height="24"
